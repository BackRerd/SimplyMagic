...
public class CareerSelectButtonMessage {
	...
	public static void handleButtonAction(Player entity, int buttonID, int x, int y, int z) {
		...
		HashMap guistate = CareerSelectMenu.guistate;
		// security measure to prevent arbitrary chunk generation
		if (!world.hasChunkAt(new BlockPos(x, y, z)))
			return;
		if (buttonID == 0) {

			CareerSelectUpButtonProcedure.execute(entity);
		}
		if (buttonID == 1) {
	...
	public static void registerMessage(FMLCommonSetupEvent event) {
		SimplymagicMod.addNetworkMessage(CareerSelectButtonMessage.class, CareerSelectButtonMessage::buffer, CareerSelectButtonMessage::new, CareerSelectButtonMessage::handler);
	}
}
